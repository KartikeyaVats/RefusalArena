...
# ==========================================
# IMPORTANT: Llama 3.2 is a gated model. You must:
# 1. Accept the license at: https://huggingface.co/meta-llama/Llama-3.2-3B-Instruct
# 2. Log in to Hugging Face by running: !huggingface-cli login
...
# ==========================================
# UPDATED: Changed to Llama 3.2 3B Instruct
...
# Llama 3.2 often requires explicit padding token definition
# We set pad_token to eos_token_id (common practice for Llama 3 families)
...
class SteeredGenerator:
    ...
    def format_llama3(self, system_prompt, user_prompt):
        """
        Formats the input using Llama 3.X specific special tokens.
        Llama 3.2 uses the same special tokens as 3.0/3.1 for standard chat.
        Structure:
    ...
    def generate_steered(self, prompt, alpha):
        # 1. Format Inputs (Using Llama 3 formatting)
        ...
        # <|eot_id|> (128009) or <|end_of_text|> (128001) are standard
        stop_tokens = [
...
print(f"\n--> Starting Alpha Sweep with Angle Analysis on Llama 3.2 3B...")
...
output_file = "alpha_sweep_llama3.2_metrics_3b_positive_alpha.csv"
